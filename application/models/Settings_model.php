...
class Settings_model extends CI_Model
{
  ...
  /*------------------------------------------------------------------------
    ...
    public function get_total_qty_alerts() {
        $this->db->where('quantity < alert_quantity', NULL, FALSE)->where('alert_quantity >', 0)->where('isDeleted', 0);
    ...
    public function getAllBarcodeConfigs() {
        $q = $this->db->get('barcode_configs');
        if ($q->num_rows() > 0) {
            return $q->result();
        }
        return array();
